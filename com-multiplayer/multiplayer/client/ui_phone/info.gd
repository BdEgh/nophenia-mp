...
func _update_info() -> void:
	...
	if client.socket and client.socket.connected():
		info_text += "[color=green]● Connected[/color]\n\n"
	else:
		info_text += "[color=red]● Disconnected[/color]\n\n"
	info_text += "[/center]"
	
	info_text += "[color=000000b0]Server[/color]: %s\n\n" % client.url
	
	if puppet_manager:
		info_text += "[color=000000b0]Players:[/color]\n"
		info_text += " • %s%s\n" % [puppet_manager.NAMES[self_id % len(puppet_manager.NAMES)], "(you)"]
		
		var player_count = puppet_manager.get_puppet_count()
		if player_count > 0:
