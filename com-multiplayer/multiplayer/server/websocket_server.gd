...
func start_server():
	...
	if error != OK:
		ModLoaderLog.error("Failed to start server on %s:%d - Error: %d" % [host, port, error], self.name)
		return
	
	ModLoaderLog.info("WebSocket server started on %s:%d" % [host, port], self.name)
