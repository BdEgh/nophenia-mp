...
func _update_look_at(delta: float):
	if look_at_target:
		if indicator_lookat and indicator_marker:
			var target_pos = look_at_target.global_position + Vector3(0, 1.2, 0)
			var local_target = indicator_marker.to_local(target_pos)
			indicator_marker.position.x = lerp(indicator_marker.position.x, clamp(local_target.x * 0.5, -0.004, 0.004), delta * 2.0)
			indicator_marker.position.z = lerp(indicator_marker.position.z, clamp(local_target.z * 0.5, 0.008, 0.012), delta * 2.0)
